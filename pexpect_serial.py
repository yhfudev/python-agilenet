...
class SerialSpawn(spawn):
    ...
    def __init__(self, ser, args=None, **kwargs):
        ...
        self.ser = ser
        self.name = '<serial port %s>' % self.ser.port
        self.child_fd = self.ser.fileno()
        self.closed = False

    def __init__(self, serial_path, serial_baud, args=None, **kwargs):
        super().__init__(None, **kwargs)
        self.ser = serial.Serial(serial_path, serial_baud, timeout=0)
        self.name = '<serial port %s>' % self.ser.port
        self.child_fd = self.ser.fileno()
        self.closed = False
